...
- stage 폴더나 snapshot run 폴더 하나만 압축해서 전달해도 그대로 열어볼 수 있는 self-contained 번들을 유지합니다
- 생성된 HTML은 로컬 서버 없이 `file://` 또는 Finder/Explorer에서 바로 열 수 있습니다
...
이 fallback 이 걸릴 때도 필요한 snapshot 이미지를 stage 폴더 안으로 materialize 하므로 `ui-evidence/screenshots/<stage-id>/` 폴더 하나만 따로 전달해도 review 가 깨지지 않습니다.

이제는 `config.mobile-ko.yaml` 같은 파생 config 대신 profile 과 params 필터로 한 config 안에서 선택할 수 있습니다.
...
생성된 HTML은 브라우저에서 파일 그대로 열면 됩니다. 로컬 서버는 필요하지 않습니다.
...
Claude plugin mirror 는 `prepare` 단계에서 canonical skill source 로부터 `plugins/ui-evidence/` 아래에 생성되고, marketplace 인덱싱이 canonical source path 하나만 보도록 의도적으로 커밋하지 않습니다.
...
durable state 와 임시 worktree 는 canonical runtime layout 아래에 정리됩니다.
