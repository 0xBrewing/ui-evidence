...
test('scaffoldConsumerRepo writes bootstrap files for Claude and Codex', async () => {
  ...
  try {
    ...
    const localInstallDoc = await readFile(path.join(tempDir, 'ui-evidence', 'installation.md'), 'utf8');
    ...
    assert.match(localInstallDoc, /ui-evidence\/config\.yaml/);
    assert.match(claudeCommand, /ui-evidence\/installation\.md/);
...
test('scaffoldConsumerRepo fails fast on legacy root layout when canonical defaults are requested', async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'ui-evidence-install-legacy-'));
  ...
  try {
    ...
    await assert.rejects(
      ...
      /Legacy ui-evidence paths detected/,
    );
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
